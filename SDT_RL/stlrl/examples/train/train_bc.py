...
def train(args: BCTrainConfig):
    ...
    process_bc_dataset(data, args.cost_limit, args.gamma, True, args.bc_mode, args.task)
    ...
    for step in trange(args.update_steps, desc="Training"):
        ...
        if (step + 1) % args.eval_every == 0 or step == args.update_steps - 1:
            ret, cost, length, safe_rate = trainer.evaluate(args.eval_episodes, args.task)
            logger.store(tab="eval", Cost=cost, Reward=ret, Length=length, SafeRate=safe_rate)
