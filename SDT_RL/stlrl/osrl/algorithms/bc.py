...
class BCTrainer:
    ...
    def evaluate(self, eval_episodes: int, task: str):
        """
        ...
        for _ in trange(eval_episodes, desc="Evaluating...", leave=False):
            epi_ret, epi_len, epi_cost = self.rollout(task)
            episode_rets.append(epi_ret)
            episode_lens.append(epi_len)
            episode_costs.append(epi_cost)
        safe_rate = np.sum(np.array(episode_costs) <= 0.0) / eval_episodes
        self.model.train()
        return np.mean(episode_rets), np.mean(episode_costs), np.mean(episode_lens), safe_rate

    @torch.no_grad()
    def rollout(self, task:str):
        """
        ...
        for _ in range(self.model.episode_len):
            ...
            if "Circle" in task:
                ...
                if cost == 1.0:
                    consecutive_ones += 1
                    new_cost = 1 if consecutive_ones > 5 else 0
                else:
                    consecutive_ones = 0
                    new_cost = 0
                cost = new_cost
            if "Run" in task:
                ...
                if cost_vel == 1.0:
                    ...
                    new_cost = 0
                cost_vel = new_cost
                cost = np.min([1, cost_bound + cost_vel])
