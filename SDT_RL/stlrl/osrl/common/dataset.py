...
from stlcg.stlcg import STL_Formula, Always, LessThan, GreaterThan, And, Implies, Negation, Eventually
...
def relabel_cost_run(obs: np.ndarray, task: str, threshold: int=5):
    ...
    if "Drone" in task or "Ant" in task:
        xdot += 1; ydot += 1
    ya = obs[:, 1]
    va = np.hypot(obs[:, xdot], obs[:, ydot])

    cost_bound = np.zeros(obs.shape[0], dtype=obs.dtype)
    cost_vel = np.zeros_like(cost_bound)
    cost_bound[np.abs(ya) > 0.2] = 1.0
    cost_vel[va > v_lim[task]] = 1.0

    new_cost_vel = relabel_cost(cost_vel, threshold)
...
def process_rvs_dataset(dataset: dict, reward_scale: float=1.0, cost_scale: float=1.0,
    ...
    """
    Processes a givne dataset for q-learning methods and rvs.
    ...
    Args:
        ...
        label_cost (bool): relabel the cost according to specification.
    ...
    dataset["returns"] = np.zeros_like(dataset["rewards"])
    if rvs_mode == "RvS-RCR":
        dataset["cost_rob_prefix"] = np.zeros_like(dataset["costs"])
        dataset["cost_rob_suffix"] = np.zeros_like(dataset["costs"])

    # compute episode returns
    for i in trange(done_idx.shape[0], desc="Processing roubstness value ..."):
        start = 0 if i == 0 else done_idx[i - 1] + 1
        end = done_idx[i] + 1
        # compute the cost and reward returns for the segment
        if label_cost:
            if "Circle" in task:
                dataset["costs"][start:end] = relabel_cost(
                    dataset["costs"][start:end], 5)
            if "Run" in task:
        ...
        dataset["cost_returns"][start:end] = cost_returns * cost_scale
        dataset["returns"][start:end] = reward_returns * reward_scale
        if rvs_mode == "RvS-RCR":
            ...
            if "Circle" in task:
                ...
                cost_inputs_prefix = ((xa, xa), ((xa, xa)))
                cost_inputs_suffix = ((xa_flip, xa_flip), ((xa_flip, xa_flip)))
            ...
            # prefix
            cost_rob_prefix = spec.forward(cost_inputs_prefix)
            cost_rob_prefix = cost_rob_prefix[0, :, 0].numpy()
            dataset["cost_rob_prefix"][start:end] = cost_rob_prefix
            # suffix
            cost_rob_suffix = spec.forward(cost_inputs_suffix)
            cost_rob_suffix = cost_rob_suffix[0, :, 0].numpy()
            cost_rob_suffix = np.flip(cost_rob_suffix)
            dataset["cost_rob_suffix"][start:end] = cost_rob_suffix

    if rvs_mode == "RvS-R":
        # s, r
        dataset["observations"] = np.hstack(
            (dataset["observations"], dataset["returns"].reshape(-1, 1)))
    elif rvs_mode == "RvS-RC":
        # s, r, c
        dataset["observations"] = np.hstack(
            (dataset["observations"], dataset["returns"].reshape(-1, 1),
             dataset["cost_returns"].reshape(-1, 1)))
    elif rvs_mode == "RvS-RCR":
        # s, r, c_suf, c_pre
        dataset["observations"] = np.hstack(
            (dataset["observations"], dataset["returns"].reshape(-1, 1),
             dataset["cost_rob_suffix"].reshape(-1, 1),
             dataset["cost_rob_prefix"].reshape(-1, 1)))
    else:
        pass


def process_bc_dataset(dataset: dict, cost_limit: float, gamma: float, 
                       label_cost: bool, bc_mode: str, task: str):
    """
    ...
    for i in range(done_idx.shape[0]):
        ...
        dataset["returns"][start:end] = reward_returns[0]
    ...
    if bc_mode == "frontier":
        ...
        pf_mask = np.logical_and(
            pf_rew_ret - (rmax - rmin) / 5 <= dataset["returns"],
            dataset["returns"] <= pf_rew_ret + (rmax - rmin) / 5)
...
def process_sequence_dataset(dataset: dict, cost_reverse: bool = False, task: str = None):
    '''
    ...
    for i in trange(dataset["rewards"].shape[0], desc="Processing trajectories"):
        ...
        if dataset["terminals"][i] or dataset["timeouts"][i]:
            ...
            if episode_data["returns"][0] > max_rew and episode_data["cost_returns"][0] == 0.0:
                max_rew = episode_data["returns"][0]
            ...
            data_, episode_step = defaultdict(list), 0
        episode_step += 1
    print(f"maximum reward of safe trajectories = {max_rew}")
...
def compute_robustness_trace(trajs: list, 
                             specification: Tuple[STL_Formula, STL_Formula], 
                             pscale: int = 1, scale: int = -1, task: str = None):
    '''
    Compute robustness value of STL specification of a list of trajectories.
    
    Args:

        trajs (list): A list of trajectories
        specification (Tuple[STL_Formula, STL_Formula]): specification for reward and cost
    ...
    ϕ_reward, ϕ_cost = specification
    if ϕ_reward is None and ϕ_cost is None or task is None:
        return trajs
    
    episode_len = 0
    for i in trange(len(trajs), desc="Computing robustness trace", leave=True):
        obs = trajs[i]["observations"]
        T = obs.shape[0]
        if T > episode_len:
            episode_len = T

        '''depend on env and states'''
        if "Circle" in task:
            xa = obs[:, 0]
            # prefix_robs, suffix_robs = compute_rob_circle(xa, 0.6, 5)
            xa = torch.tensor(xa, requires_grad=False).reshape([1, xa.shape[0], 1])
            xa_flip = xa.flip(1)
        if "Run" in task:
            xdot = 2; ydot = 3
        ...
        if ϕ_cost is not None:
            ...
            cost_rob_prefix = cost_rob_prefix[0, :, 0].numpy()
            trajs[i]["cost_rob_prefix"] = cost_rob_prefix

            # cost_rob_suffix
            cost_rob_suffix = ϕ_cost.forward(cost_inputs_suffix, pscale, scale)
            cost_rob_suffix = cost_rob_suffix[0, :, 0].numpy()
            cost_rob_suffix = np.flip(cost_rob_suffix)
            trajs[i]["cost_rob_suffix"] = cost_rob_suffix
...
def compute_cost_sample_prob(dataset, cost_transform=lambda x: 50 - x, 
                             use_cost_suffix=False, use_cost_prefix=False):
    """
    ...
    for i, traj in enumerate(dataset):
        if use_cost_suffix:
            c = cost_transform(traj["cost_rob_suffix"][0])
        elif use_cost_prefix:
            c = cost_transform(traj["cost_rob_prefix"][-1])
        else:
...
class SequenceDataset(IterableDataset):
    ...
    Args:
        ...
        specification: Tuple[LinearPredicate, LinearPredicate] = (None, None),
        pscale: int = 1,
        rob_scale: int = -1,
    ...
    def __init__(
        ...
        self.original_data, info = process_sequence_dataset(dataset, cost_reverse, task)
        self.ϕ_reward, self.ϕ_cost = specification
        self.original_data = compute_robustness_trace(
                self.original_data, specification, pscale, rob_scale, task)
        ...
        if cost_sample:
            self.sample_prob = compute_cost_sample_prob(self.dataset, cost_transform, 
                                                        self.use_cost_suffix, self.use_cost_prefix)
        elif pf_sample:
    ...
    def __prepare_sample(self, traj_idx, start_idx):
        ...
        # rew
        rew_key = "rew_rob_suffix" if self.use_rew_suffix else "returns"
        rew_rob_suffix = traj[rew_key][start_idx:start_idx + self.seq_len]
        rew_rob_prefix = traj["rew_rob_prefix"][start_idx:start_idx + self.seq_len] if self.use_rew_prefix else None
        # cost
        cost_key = "cost_rob_suffix" if self.use_cost_suffix else "cost_returns"
        cost_rob_suffix = traj[cost_key][start_idx:start_idx + self.seq_len]
        cost_rob_prefix = traj["cost_rob_prefix"][start_idx:start_idx + self.seq_len] if self.use_cost_prefix else None
        ...
        rew_rob_suffix = rew_rob_suffix * self.reward_scale
        rew_rob_prefix = rew_rob_prefix * self.reward_scale if rew_rob_prefix is not None else rew_rob_prefix
        cost_rob_suffix = cost_rob_suffix * self.cost_scale
        cost_rob_prefix = cost_rob_prefix * self.cost_scale if cost_rob_prefix is not None else cost_rob_prefix
        ...
        if states.shape[0] < self.seq_len:
            ...
            rew_rob_suffix = pad_along_axis(rew_rob_suffix, pad_to=self.seq_len)
            rew_rob_prefix = pad_along_axis(rew_rob_prefix, pad_to=self.seq_len) \
                if rew_rob_prefix is not None else rew_rob_prefix
            cost_rob_suffix = pad_along_axis(cost_rob_suffix, pad_to=self.seq_len)
            cost_rob_prefix = pad_along_axis(cost_rob_prefix, pad_to=self.seq_len) \
                if cost_rob_prefix is not None else cost_rob_prefix
            costs = pad_along_axis(costs, pad_to=self.seq_len)

        if self.use_cost_prefix and self.use_rew_prefix:
            return states, actions, (rew_rob_prefix, rew_rob_suffix), (cost_rob_prefix, cost_rob_suffix), time_steps, mask, episode_cost, costs
        elif self.use_cost_prefix and not self.use_rew_prefix:
            return states, actions, rew_rob_suffix, (cost_rob_prefix, cost_rob_suffix), time_steps, mask, episode_cost, costs
        elif not self.use_cost_prefix and self.use_rew_prefix:
            return states, actions, (rew_rob_prefix, rew_rob_suffix), cost_rob_suffix, time_steps, mask, episode_cost, costs
        else:
            return states, actions, rew_rob_suffix, cost_rob_suffix, time_steps, mask, episode_cost, costs
    ...
    def __iter__(self):
        while True:
            ...
            if self.start_sampling:
                ...
                # for circle envs, there's big change between obs[0] nad obs[1],
                # so change 0 to 1 to ommit this data
